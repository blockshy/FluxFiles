...
func New() (*App, error) {
	...
	httpRouter := router.New(router.Dependencies{
		...
		PublicFiles:     controller.NewPublicFileController(fileService, userService, taxonomyService, settingsService, captchaService),
		...
		AdminDownloads:  controller.NewAdminDownloadController(userService),
